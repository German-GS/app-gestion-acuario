...
const AddReminderModal = ({ visible, onClose, onSave }) => {
  ...
  // Estado extra para controlar si mostramos 'date' o 'time' en Android
  ...
  const onChangeDate = (event, selectedDate) => {
    ...
    if (Platform.OS === "android") {
      setShowDatePicker(false); // Cerramos el picker actual
      setDate(currentDate); // Guardamos lo que seleccionó (fecha u hora)

      // Lógica de secuencia para Android: Si acaba de elegir fecha, mostramos hora.
      if (event.type === "set") {
        // Solo si el usuario dio a "Aceptar"
        if (androidMode === "date") {
          setAndroidMode("time"); // Cambiamos modo a hora
          setTimeout(() => setShowDatePicker(true), 50); // Abrimos el picker de hora
        } else {
          setAndroidMode("date"); // Reseteamos para la próxima vez
        }
      } else {
        // Si canceló, reseteamos al modo fecha
        setAndroidMode("date");
      }
    } else {
      // En iOS el picker se queda abierto o se maneja distinto
      setDate(currentDate);
    }
  };
  ...
  const showDatepicker = () => {
    if (Platform.OS === "android") {
      setAndroidMode("date"); // En Android siempre empezamos pidiendo la fecha
    }
      ...
      <View style={styles.centeredView}>
        <View style={styles.modalView}>
          ...
          <View style={styles.pickerContainer}>
            <Picker
              selectedValue={task}
              onValueChange={(itemValue) => setTask(itemValue)}
            >
          ...
          {showDatePicker && (
            <DateTimePicker
              ...
              value={date}
              // En Android usa el estado 'androidMode' (date o time), en iOS usa 'datetime'
